...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnInsertToProvider.setOnClickListener {
            insertNewRecipe(etName.text.toString(),
                    etCont.text.toString())
        }
    ...
    private fun insertNewRecipe(recipe: String, content: String): Uri? {
        val values = ContentValues()
        values.put(RecipeContract.RecipeEntry.COLUMN_DATE,
                Date(System.currentTimeMillis()).toString())
        values.put(RecipeContract.RecipeEntry.COLUMN_TITLE, recipe)
        values.put(RecipeContract.RecipeEntry.COLUMN_DESC, content)
